...
export default function WithNavPill({
  brand = 'tailark',
  signInLabel = 'Sign In',
  ...
  return (
    <div ref={navRef} className="relative z-50 bg-white dark:bg-gray-950">
      ...
      >
        {/* flex-1 on both outer cells is what actually centres the pill: with
            `justify-between` alone the pill sits at the midpoint of the gap
            between two unequal-width cells, which is visibly off-centre. */}
        ...
        <div className="hidden items-center gap-0.5 rounded-2xl border border-gray-200/80 bg-gray-50/80 p-1.5 backdrop-blur-xl lg:flex dark:border-white/10 dark:bg-white/[0.04]">
          {MENUS.map((label) => {
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-1.5 rounded-xl px-3.5 py-2 text-sm font-medium transition-all duration-200 ${
                  active
                    ? 'bg-white text-gray-900 shadow-[0_1px_2px_rgb(15_23_42_/_0.08)] dark:bg-white/10 dark:text-white dark:shadow-none'
          ...
          {LINKS.map((label) => (
            <a
              key={label}
              href="#"
              className="rounded-xl px-3.5 py-2 text-sm font-medium text-gray-700 transition-colors duration-200 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white"
            >
              {label}
            </a>
          ))}
      ...
      {/* Centred on the viewport, matching the pill above it. */}
      {openMenu && (
        <div className="absolute inset-x-0 top-full hidden px-6 lg:block">
          <div
            className={`mx-auto grid max-w-3xl grid-cols-[1.25fr_1fr] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-[0_16px_48px_-12px_rgb(15_23_42_/_0.18)] transition-all duration-200 ease-out dark:border-white/10 dark:bg-gray-900 ${
